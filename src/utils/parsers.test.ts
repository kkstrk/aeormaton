...
describe('parseTikTokItems', () => {
    ...
    it('should replace mentions in post text', () => {
        const result = parseTikTokItems([
            ...
            {  ...mockItem, title: 'Replaces Brennan Lee Mulligan w/ @brennanleemulligan.bsky.social.'},
        ]);
        assert.equal(result[0].text, 'Replaces @laurabaileyvo.bsky.social w/ @laurabaileyvo.bsky.social.');
        assert.equal(result[1].text, 'Replaces Marisha Ray w/ Marisha Ray.');
        assert.equal(result[2].text, 'Replaces @brennanleemulligan.bsky.social w/ @brennanleemulligan.bsky.social.');
    });
});
...
describe('parseNewsItems', () => {
    ...
    it('should add bsky handles for CR members', () => {
        ...
        assert.equal(result[2].text, 'A post w/ Matthew Mercer (@matthewmercer.bsky.social) and Matthew Mercer');
        assert.equal(result[3].text, 'A post w/ Laura Bailey (@laurabaileyvo.bsky.social) and Matthew Mercer (@matthewmercer.bsky.social)');
    });
